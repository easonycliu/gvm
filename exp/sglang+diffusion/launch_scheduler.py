...
if __name__ == "__main__":
	...
	while (True):
		...
		if (time.time() > operate_time + OPERATE_INTERVAL_MIN_MS / 1000):
			if sum(nr_pending_kernels_list[-SLIDE_WINDOW_SIZE:]) == 0 and len(set(nr_submitted_kernels_list[-SLIDE_WINDOW_SIZE:])) == 1:
				pass
				#print(nr_submitted_kernels_list[-SLIDE_WINDOW_SIZE:])
