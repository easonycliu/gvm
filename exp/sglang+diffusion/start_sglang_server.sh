...
model=
device=
param="--mem-fraction-static 0.75 --attention-backend torch_native --disable-cuda-graph --log-level warning"
...
if [ "$method" == "GVM" ]; then
	...
	source $project_dir/playground/infer/venv/sglang/bin/activate
	python3 -m sglang.launch_server --model-path $model $param &
elif [ "$method" == "UVM" ]; then
	source $project_dir/playground/infer/venv/sglang/bin/activate
	LD_PRELOAD="$project_dir/cuda_custom/libcustom_cuda.so" python3 -m sglang.launch_server --model-path $model $param &
...
if [ -n $pidfile ]; then
	while true; do
		if nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv | grep "sglang"; then
			sglang_active_process=$(nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv | grep "sglang" | awk -F "," '{print $1}')
			echo "sglang process detected at $sglang_active_process!"
			echo $sglang_active_process | tee $pidfile
			break
		fi
		sleep 1
	done
